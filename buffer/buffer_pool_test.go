...
func TestBufferPool(t *testing.T) {
	...
	convey.Convey("Buffer Pool Test: ", t, func() {
		...
		convey.Convey("Buffer pool serial test:", func() {
			// For purpose of boundary condition test, have to allocate the first half at the beginning
		...
		convey.Convey("Buffer pool release test: should pass.", func() {
			for i := 0; i < 10; i++ {
				...
				convey.So(md5temp, convey.ShouldEqual, md5.Sum(tempBufferRead))
			}
		})
		...
		convey.Convey("Buffer pool concurrent test: should pass.", func() {
			for i := 0; i < testPool.MaxCap(); i++ {
				go func(count int) {
					...
					sendUnit.writeLength, sendUnit.writeErr = sendUnit.buffer.Write(tempBufferWrite)
			...
			for receiveUnit := range testChannel {
				...
				convey.So(receiveUnit.writeLength, convey.ShouldEqual, testPool.BufferCap()*1024)
